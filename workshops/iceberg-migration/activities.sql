...
--  (modify tmp_first_last_postalcode and 99999999 as appropriate)
CALL tmp_cat.system.rollback_to_snapshot(
   'tmp_first_last_postalcode', 'dune_characters',
    99999999);
SELECT * FROM dune_characters;
...
-- stand up newly rewritten table and notice that
...
AS SELECT * FROM orders_hive_orc_bucketed;
--
...
------------------------------------
-- setup start --
DROP TABLE IF EXISTS cust_hive_parquet_part;
CREATE TABLE cust_hive_parquet_part
WITH (type='hive', format='parquet', 
...
SHOW CREATE TABLE cust_hive_parquet_part;
SELECT * FROM "cust_hive_parquet_part$partitions";
SELECT format_number(COUNT()) FROM cust_hive_parquet_part;

-- 1. CREATE A NEW TABLE
CREATE TABLE cust_hive_parquet_part_NEW (
   custkey bigint,
   name varchar(25),
   address varchar(40),
   nationkey bigint,
...
-- 2. RENAME OLD TABLE
ALTER TABLE cust_hive_parquet_part 
RENAME TO cust_hive_parquet_part_OLD;

-- 3. CREATE A VIEW
CREATE VIEW cust_hive_parquet_part AS
SELECT * FROM cust_hive_parquet_part_NEW
UNION ALL
SELECT * FROM cust_hive_parquet_part_OLD;
--  verify it works
SELECT format_number(COUNT()) FROM cust_hive_parquet_part;

-- remind ourselves of the partition names
SELECT * FROM "cust_hive_parquet_part_old$partitions";

-- 4. (1st pass -- AUTOMOBILE)
-- 4A insert partition into new
INSERT INTO cust_hive_parquet_part_NEW 
SELECT * FROM cust_hive_parquet_part_OLD
 WHERE mktsegment = 'AUTOMOBILE';
-- 4B drop partition from old
DELETE FROM cust_hive_parquet_part_OLD
 WHERE mktsegment = 'AUTOMOBILE';
...
-- 4D make sure there is still only 1.5M rows
SELECT format_number(COUNT()) FROM cust_hive_parquet_part;
...
-- 5. DROP THE VIEW
DROP VIEW cust_hive_parquet_part;
-- 6. DROP THE OLD TABLE
DROP TABLE cust_hive_parquet_part_OLD;
-- 7. RENAME NEW TO ORIGINAL NAME
ALTER TABLE cust_hive_parquet_part_NEW 
RENAME TO cust_hive_parquet_part;
